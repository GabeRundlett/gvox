...
extern "C" void gvox_input_adapter_file_begin(GVoxAdapterContext *ctx, void *config) {
    auto &user_state = *reinterpret_cast<FileState *>(gvox_input_adapter_malloc(ctx, sizeof(FileState)));
...
extern "C" void gvox_input_adapter_file_read(GVoxAdapterContext *ctx, size_t position, size_t size, void *data) {
    ...
    if (ret != 0) {
        gvox_input_adapter_push_error(ctx, GVOX_ERROR_UNKNOWN, "Failed to set the file read head");
        return;
    }
