...
extern "C" void gvox_parse_adapter_gvox_raw_begin(GvoxAdapterContext *ctx, [[maybe_unused]] void *config) {
    ...
    uint32_t magic = 0;
    gvox_input_read(ctx, user_state.offset, sizeof(uint32_t), &magic);
    ...
    gvox_input_read(ctx, user_state.offset, sizeof(GvoxRegionRange), &user_state.range);
...
extern "C" auto gvox_parse_adapter_gvox_raw_load_region(GvoxAdapterContext *ctx, GvoxOffset3D const *offset, uint32_t channel_id) -> GvoxRegion {
    auto &user_state = *reinterpret_cast<GvoxRawParseUserState *>(gvox_parse_adapter_get_user_pointer(ctx));
...
extern "C" void gvox_parse_adapter_gvox_raw_unload_region([[maybe_unused]] GvoxAdapterContext *ctx, [[maybe_unused]] GvoxRegion *region) {
}

extern "C" auto gvox_parse_adapter_gvox_raw_sample_region([[maybe_unused]] GvoxAdapterContext *ctx, GvoxRegion const *region, [[maybe_unused]] GvoxOffset3D const *offset, uint32_t /*channel_id*/) -> uint32_t {
