...
extern "C" void gvox_serialize_adapter_colored_text_begin([[maybe_unused]] GVoxAdapterContext *ctx, [[maybe_unused]] void *config) {
    ...
    gvox_serialize_adapter_set_user_pointer(ctx, &user_state);
    if (config != nullptr) {
...
extern "C" void gvox_serialize_adapter_colored_text_serialize_region(GVoxAdapterContext *ctx, GVoxRegionRange const *range) {
    auto &user_state = *reinterpret_cast<UserState *>(gvox_serialize_adapter_get_user_pointer(ctx));
                        ...
                        for (uint32_t sub_yi = 0; sub_yi < sub_n && (yi + sub_yi < range->extent.y); ++sub_yi) {
                            for (uint32_t sub_xi = 0; sub_xi < sub_n && (xi + sub_xi < range->extent.x); ++sub_xi) {
                                auto const pos = GVoxOffset3D{
                                    ...
                                    static_cast<int32_t>(range->extent.z + range->offset.z - (zi + sub_zi) - 1),
                                };
                                auto voxel = gvox_sample_data(ctx, &pos, user_state.config.channel_index);
                                if (is_3channel) {
