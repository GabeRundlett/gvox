...
extern "C" void gvox_serialize_adapter_colored_text_serialize_region(GvoxBlitContext *blit_ctx, GvoxAdapterContext *ctx, GvoxRegionRange const *range, uint32_t channel_flags) {
    ...
    auto line_terminator = std::to_array("\033[0m ");
    if (user_state.config.vertical) {
        line_terminator[line_terminator.size() - 2] = '\n';
    }
    ...
    bool is_3channel = false;
    bool is_normalized_float = false;
    ...
    for (auto temp_channel_id : channels) {
        ...
        if (user_state.config.vertical) {
            for (yi = 0; yi < range->extent.y; yi += user_state.config.downscale_factor) {
                for (zi = 0; zi < range->extent.z; zi += user_state.config.downscale_factor) {
                    do_inner_loop();
                }
