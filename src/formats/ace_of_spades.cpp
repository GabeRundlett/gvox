    ...
    for (j = 0; j < scene.nodes[0].size_x; ++j) {
        for (i = 0; i < scene.nodes[0].size_y; ++i) {
            ...
            while (k < MAP_Z) {
                size_t z = 0;
                size_t air_start = 0;
                size_t top_colors_start = 0;
                size_t top_colors_end = 0; // exclusive
                size_t bottom_colors_start = 0;
                size_t bottom_colors_end = 0; // exclusive
                size_t top_colors_len = 0;
                size_t bottom_colors_len = 0;
                size_t colors = 0;
                // find the air region
                air_start = k;
                while (k < MAP_Z && (sample_map(i, j, k) == 0u)) {
                    ++k;
                }
                // find the top region
                top_colors_start = k;
                ...
                while (k < MAP_Z && (sample_map(i, j, k) != 0u) && !is_surface(i, j, k)) {
                    ++k;
                }
...
auto AceOfSpadesContext::parse_payload(GVoxPayload payload) -> GVoxScene {
    ...
    auto set_color = [&result](size_t x, size_t y, size_t z, uint32_t u32_voxel) {
        ...
        float const r = static_cast<float>((u32_voxel >> 0x10) & 0xff) / 255.0f;
        float const g = static_cast<float>((u32_voxel >> 0x08) & 0xff) / 255.0f;
        float const b = static_cast<float>((u32_voxel >> 0x00) & 0xff) / 255.0f;
        auto voxel_i = x + (result.nodes[0].size_y - 1 - y) * result.nodes[0].size_x + (result.nodes[0].size_z - 1 - z) * result.nodes[0].size_x * result.nodes[0].size_y;
    ...
    for (y = 0; y < result.nodes[0].size_y; ++y) {
        for (x = 0; x < result.nodes[0].size_x; ++x) {
            ...
            for (;;) {
                ...
                size_t const number_4byte_chunks = v[0];
                size_t const top_color_start = v[1];
                size_t const top_color_end = v[2];
                size_t bottom_color_start = 0;
                size_t bottom_color_end = 0;
                size_t len_top = 0;
                size_t len_bottom = 0;
...
extern "C" EXPORT auto gvox_format_ace_of_spades_create_payload(void *context_ptr, GVoxScene const *scene) -> GVoxPayload {
    auto *self = reinterpret_cast<AceOfSpadesContext *>(context_ptr);
    return self->create_payload(*scene);
}

extern "C" EXPORT void gvox_format_ace_of_spades_destroy_payload(void *context_ptr, GVoxPayload const *payload) {
    auto *self = reinterpret_cast<AceOfSpadesContext *>(context_ptr);
    self->destroy_payload(*payload);
}

extern "C" EXPORT auto gvox_format_ace_of_spades_parse_payload(void *context_ptr, GVoxPayload const *payload) -> GVoxScene {
    auto *self = reinterpret_cast<AceOfSpadesContext *>(context_ptr);
