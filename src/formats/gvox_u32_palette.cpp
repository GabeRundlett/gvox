...
struct PaletteCompressor {
    ...
    auto region_variance(GVoxSceneNode const &node, size_t region_x, size_t region_y, size_t region_z) -> size_t {
        ...
        if (variants > 367) {
            ...
            auto *palette_end = palette_begin + variants;
                        ...
                        for (size_t zi = 0; zi < REGION_SIZE; ++zi) {
                            for (size_t yi = 0; yi < REGION_SIZE; ++yi) {
                                for (size_t xi = 0; xi < REGION_SIZE; ++xi) {
                                    ...
                                    auto const pz = oz + zi;
                                    auto const index = px + py * node.size_x + pz * node.size_x * node.size_y;
                                    auto const u32_voxel = *reinterpret_cast<uint32_t *>(buffer_ptr + index * sizeof(std::uint32_t));
...
extern "C" EXPORT auto gvox_format_gvox_u32_palette_create_payload(void *context_ptr, GVoxScene const *scene) -> GVoxPayload {
    auto *self = reinterpret_cast<GVoxU32PaletteContext *>(context_ptr);
    return self->create_payload(*scene);
}

extern "C" EXPORT void gvox_format_gvox_u32_palette_destroy_payload(void *context_ptr, GVoxPayload const *payload) {
    auto *self = reinterpret_cast<GVoxU32PaletteContext *>(context_ptr);
    self->destroy_payload(*payload);
}

extern "C" EXPORT auto gvox_format_gvox_u32_palette_parse_payload(void *context_ptr, GVoxPayload const *payload) -> GVoxScene {
    auto *self = reinterpret_cast<GVoxU32PaletteContext *>(context_ptr);
