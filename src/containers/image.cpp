#include <gvox/gvox.h>
#include <gvox/containers/image.h>
...
struct GvoxImageContainer {
    GvoxImageContainerConfig config{};
    std::vector<ImageLayer> layers{};
    AABB bounds{};

    GvoxImageContainer(GvoxImageContainerConfig const &a_config) : config{a_config} {}
        ...
        for (int yi = floor(bounds.min_y); yi < ceil(bounds.max_y); ++yi) {
            for (int xi = floor(bounds.min_x); xi < ceil(bounds.max_x); ++xi) {
                ...
                for (auto const &layer : layers) {
                    ...
                    // auto inv_transform1 = layer.inv_transform;
                    auto pt = gvox_apply_transform(&inv_transform, GvoxTranslation{{static_cast<float>(xi), static_cast<float>(yi), 0.0f, 0.0f}});
                    // if (inv_transform != inv_transform1) {
                    //     std::cout << "t0" << inv_transform << std::endl;
                    //     std::cout << "t1" << inv_transform1 << std::endl;
                    // }
...
auto GvoxImageContainer::parse(GvoxParseCbInfo const &info) -> GvoxResult {
    stbi_io_callbacks callbacks;
    callbacks.read = [](void *user, char *data, int size) -> int {
        auto input_adapter = static_cast<GvoxIoAdapter>(user);
        gvox_input_read(input_adapter, reinterpret_cast<uint8_t *>(data), static_cast<size_t>(size));
        return size;
    };
    callbacks.skip = [](void *user, int n) {
        auto input_adapter = static_cast<GvoxIoAdapter>(user);
        gvox_input_seek(input_adapter, n, GVOX_SEEK_ORIGIN_CUR);
    };
    ...
    auto *data = stbi_load_from_callbacks(&callbacks, info.input_adapter, &size_x, &size_y, &comp, 4);
    if (data == nullptr) {
...
auto GvoxImageContainer::serialize() -> GvoxResult {
    return GVOX_SUCCESS;
}
...
auto gvox_container_image_create(void **self, void const *config_ptr) -> GvoxResult {
    ...
    *self = new GvoxImageContainer(config);
    return GVOX_SUCCESS;
}
auto gvox_container_image_parse(void *self, GvoxParseCbInfo const *info) -> GvoxResult {
    return static_cast<GvoxImageContainer *>(self)->parse(*info);
}
auto gvox_container_image_serialize(void *self) -> GvoxResult {
    return static_cast<GvoxImageContainer *>(self)->serialize();
}
void gvox_container_image_destroy(void *self) {
    delete static_cast<GvoxImageContainer *>(self);
}
